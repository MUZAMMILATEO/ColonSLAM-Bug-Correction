...
class TopologicalMapSLAM:
    def __init__(self, conf, dimension, likelihood_estimator, window=None, vg=False):
        ...
        if raw is None:
            logging.warning(f"Could not load default image at {img_path}, using blank placeholder")
            # create a blank 250×200 grey image
            raw = 255 * np.ones((200, 250, 3), dtype=np.uint8)
        self.default_image = cv2.resize(raw, (250, 200))
